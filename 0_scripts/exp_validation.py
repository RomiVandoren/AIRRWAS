...
def summarize_overlap_conv(stim_data, discovery_data, conv_data, cell_name):
    ...
    for chain in chains:
        summary[f"{chain}_patient_matched"] = summary.apply(
            lambda x: (f"{x[f'{chain}_patient_matched']}"),
            axis=1,
        )
        summary[f"{chain}_overlap_conv"] = summary.apply(
            lambda x: (f"{x[f'{chain}_overlap_conv']}"),
            axis=1,
        )
        summary[f"{chain}_patient_matched_conv"] = summary.apply(
            lambda x: (f"{x[f'{chain}_patient_matched_conv']}"),
            axis=1,
        )
...
def Upset_public_tcrs(
    ...
    plt.tight_layout()
    plt.savefig(
        suppl_dir / f"S9C_D_validation1_upset_{condition_prefix}.pdf",
        bbox_inches="tight",
    )
    plt.show()
...
def plot_overlap_nested(ax, df_sorted, labels, title, annotate_percent=True):
    """
    Plot nested bars: Total TCRs (grey), discovery subset (teal), convergent subset (pink).
    Smaller bars are centered on the larger bars to show nested subsets.
    Optionally annotate percentages above each segment.
    """
    x = np.arange(len(labels))
    total_width = 0.8  # full width for the grey bar
    discovery_width = total_width * 0.8
    convergent_width = total_width * 0.6

    # Colors
    colors = {
        "Total TCRs": "#BABABA",
        "Overlap with discovery": "#0173b2",  # "#4C7778",
        "Overlap with convergent TCRs": "#de8f05",  # "#E17A89",
    }

    # Draw bars
    ax.bar(
        x,
        df_sorted["Total TCRs"],
        width=total_width,
        color=colors["Total TCRs"],
        label="Total TCRs",
    )
    ax.bar(
        x,
        df_sorted["Overlap with discovery"],
        width=discovery_width,
        color=colors["Overlap with discovery"],
        label="Overlap with discovery",
    )
    ax.bar(
        x,
        df_sorted["Overlap with convergent TCRs"],
        width=convergent_width,
        color=colors["Overlap with convergent TCRs"],
        label="Overlap with convergent TCRs",
    )

    # Annotate percentages if desired
    if annotate_percent:
        for i, row in df_sorted.iterrows():
            total = row["Total TCRs"]
            disc = row["Overlap with discovery"]
            conv = row["Overlap with convergent TCRs"]
    ...
    # Annotate total TCR counts above grey bars
    for xi, val in zip(x, df_sorted["Total TCRs"].astype(int)):
        ax.text(
            xi,
            val + max(df_sorted["Total TCRs"]) * 0.01,
            f"{val}",
            ha="center",
            va="bottom",
            fontsize=12,
    ...
    # X-axis labels
    ax.set_xticks(x)
    ax.set_xticklabels([lab.split("_")[0] for lab in labels], rotation=90, fontsize=12)
    ax.set_ylabel("Number of TCRs", fontsize=14)
    ax.set_title(title, fontsize=16, fontweight="bold")

    # Legend
    ax.legend(frameon=False, fontsize=12, bbox_to_anchor=(1.05, 1))
...
############################################
### Faceted per-patient stacked bar plot ###
############################################
def plot_MAIT_per_patient(plot_df, patients, labels, conditions, title, figures_dir):
    """
    Faceted stacked bar plot: two rows of subplots per patient
    - skips patients with no data (all zeros)
    - automatic row/column layout
    """
    # Identify patients with any data
    fancy_colors = ["#feedde", "#fdbe85", "#fd8d3c", "#e6550d"]
    valid_patients = []
    for patient in patients:
        has_data = False
        for cond in conditions:
            try:
                row = plot_df.loc[(patient, cond)]
                if row[labels].sum() > 0:
                    has_data = True
                    break
            except KeyError:
    ...
    if n_patients == 0:
        print("No patients with data to plot!")
        return
    ...
    fig, axes = plt.subplots(
        n_rows, n_cols, figsize=(4 * n_cols, 5 * n_rows), sharey=True
    )
    axes = axes.flatten()

    for i, patient in enumerate(valid_patients):
        ax = axes[i]
        bar_idx = np.arange(len(conditions))
        bottom_vals = np.zeros(len(conditions))

        for k, label_bin in enumerate(labels):
            values = []
            for cond in conditions:
                try:
                    row = plot_df.loc[(patient, cond)]
                ...
                values.append(val)
            ax.bar(
                bar_idx, values, bottom=bottom_vals, color=fancy_colors[k], width=0.8
            )
            bottom_vals += np.array(values)

        # Annotate n_unique on top of each bar
        for j, cond in enumerate(conditions):
            try:
                n_unique = plot_df.loc[(patient, cond)]["n_unique"]
            except KeyError:
                n_unique = 0
            if n_unique > 0:
                ax.text(
        ...
        ax.set_xticklabels(conditions, rotation=0, fontsize=13)
        ax.set_title(patient, fontsize=12, fontweight="bold")
        ax.set_ylim(0, max(bottom_vals) * 1.2)
    ...
    # Shared y-axis label
    fig.text(
        -0.01, 0.5, "% MAIT repertoire", va="center", rotation="vertical", fontsize=12
    )

    # Legend
    legend_elements = [
        Patch(facecolor=fancy_colors[i], label=labels[i]) for i in range(len(labels))
    ]
    fig.legend(
    ...
    plt.tight_layout()
    plt.savefig(figures_dir / "S10_MAIT_expansion.pdf", bbox_inches="tight")
    plt.show()


###############################################
### Negative network functions and plotting ###
###############################################
...
def compute_stats(val, other_vals, correction=True):
    other_vals = np.array(other_vals)
    z_score = (
        (val - np.mean(other_vals)) / np.std(other_vals, ddof=1)
        if np.std(other_vals) > 0
        else np.nan
    )
    p_zscore = 1 - norm.cdf(z_score) if not np.isnan(z_score) else np.nan

    if correction:
        p_empirical = (np.sum(other_vals >= val) + 1) / (len(other_vals) + 1)
    else:
        p_empirical = (
            np.sum(other_vals >= val) / len(other_vals)
            if len(other_vals) > 0
            else np.nan
        )

    return z_score, p_zscore, p_empirical
...
def compute_overlap_simple(network_df, lab_df, genus, network_type, shuffle_idx=None):
    df = network_df.copy()
    df["chain"] = df["v_call"].str.extract(r"(TR[AB])")
    df["full_tcr"] = df["v_call"] + "_" + df["junction_aa"] + "_" + df["j_call"]
    df = df.drop_duplicates(subset=["full_tcr", "chain", "genus"])

    results = []
    for chain in ["TRA", "TRB"]:
        lab_set = set(lab_df[lab_df["chain"] == chain]["full_tcr"])
        net_set = set(df[(df["chain"] == chain) & (df["genus"] == genus)]["full_tcr"])
        n_overlap = len(net_set & lab_set)
        n_total = len(net_set)
        results.append(
            {
                "genus": genus,
...
def compute_overlap_conv_with_stats(
    conv_df,
    lab_df,
    genus_to_check,
    remove_small=False,
    min_tcrs=1000,
    metric_col="percent_overlap",
):
    conv_df = conv_df.copy()
    conv_df["chain"] = conv_df["v_call"].str.extract(r"(TR[AB])")
    conv_nopat = conv_df.drop_duplicates(subset=["full_tcr", "chain", "genus"])
    lab_df = lab_df.drop_duplicates(
        subset=["full_tcr", "chain", "patient_id", "condition"]
    )

    results = []
    for chain in ["TRA", "TRB"]:
        lab_set = set(lab_df[lab_df["chain"] == chain]["full_tcr"])
        subset = conv_nopat[conv_nopat["chain"] == chain]
        for genus in subset["genus"].unique():
            genus_set = set(subset[subset["genus"] == genus]["full_tcr"])
            n_overlap = len(genus_set & lab_set)
            n_total = len(genus_set)
            results.append(
                {
                    "chain": chain,
                    "genus": genus,
                    ...
                    "n_overlap_with_lab": n_overlap,
                    "percent_overlap": (
                        n_overlap / n_total * 100 if n_total > 0 else 0
                    ),
                }
            )
    ...
    if remove_small:
        small_genera = overlap_df[overlap_df["n_predicted_TCRs"] <= min_tcrs][
            "genus"
        ].unique()
        small_genera = [g for g in small_genera if g != genus_to_check]
        overlap_df = overlap_df[~overlap_df["genus"].isin(small_genera)]

    # Compute stats for target genus using chosen metric
    stat_results = []
    for chain in ["TRA", "TRB"]:
        subset = overlap_df[overlap_df["chain"] == chain]
        val = subset[subset["genus"] == genus_to_check][metric_col].values[0]
        other_vals = subset[subset["genus"] != genus_to_check][metric_col].values
        z, p_z, p_emp = compute_stats(val, other_vals)
        stat_results.append(
            {
                "chain": chain,
                "genus": genus_to_check,
                "value": val,
                "z_score": z,
                "p_zscore": p_z,
                "p_empirical": p_emp,
