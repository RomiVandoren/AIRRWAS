...
def main():
    ...
    datasets_info = [
        {
            ...
            "micro_path": disc_conv_dir / "genus_count_matrix_th1.csv",
        ...
        },
        {
            ...
            "micro_path": ibd_conv_dir / "genus_count_matrix_th1.csv",
            "group_col": "patient_id",
            "chain": "alpha",
        ...
        },
        {
            ...
            "micro_path": crc_conv_dir / "genus_count_matrix_th1.csv",
    ...
    for info in datasets_info:
        info["save_dir"].mkdir(parents=True, exist_ok=True)
        info["data"] = pd.read_csv(info["data_path"]).drop_duplicates(
            subset=info["dupl"]
        )
        info["micro"] = pd.read_csv(info["micro_path"], index_col=0)

    # Handling for Prevotella / Peptoniphilus
    datasets_info[0]["micro"] = datasets_info[0]["micro"][
        ["Prevotella", "Peptoniphilus"]
    ]
    datasets_info[1]["micro"] = datasets_info[1]["micro"][
        ["Prevotella", "Peptoniphilus"]
    ]
    datasets_info[2]["micro"] = datasets_info[2]["micro"][
        ["Prevotella"]
    ]  # no Peptoniphilus
    datasets_info[3]["micro"] = datasets_info[3]["micro"][["Prevotella"]]
    datasets_info[4]["micro"] = datasets_info[4]["micro"][
        ["Prevotella", "Peptoniphilus_A", "Peptoniphilus_B"]
    ]
    datasets_info[5]["micro"] = datasets_info[5]["micro"][
        ["Prevotella", "Peptoniphilus_A", "Peptoniphilus_B"]
    ]

    # Loop over datasets and genera
    for info in datasets_info:
        for genus_name in info["micro"].columns:
            patients = info["micro"].loc[info["micro"][genus_name] > 0].index.tolist()
            logging.info(
                f"Processing genus {genus_name} for {info['chain']} chain, {len(patients)} patients"
            )

            result = get_per_gene_convergence(info["data"], info["group_col"], patients)
            result.to_csv(
                info["save_dir"] / f"X_{genus_name}_{info['chain']}_results.csv",
                index=False,
            )
